...
function Demo() {
  ...
  const [getSumarry, { error, isFetching}] = useLazyGetSummaryQuery();
  ...
  const handleSubmit = async (e) => {
    ...
    const {data} = await getSumarry({ articleUrl: article.url });
  ...
  return (
    <section className='mt-16 w-full max-w-xl'>
        ...
        <div className='my-10 max-w-full flex justify-center items-center'>
                  {isFetching ? <img src={loader} className='w-20 h-20 object-contain' /> : error ? (
                    <p className='font-inter font-bold text-black text-center'>
                      ...
                      article.summary && (
                        <div className='flex flex-col gap-3'>
                          <h2 className='font-satoshi font-vold text-gray-600 text-xl'>
